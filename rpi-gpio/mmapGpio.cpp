...
const unsigned int mmapGpio::GPPUD;
const unsigned int mmapGpio::GPPUDCLK0;
const unsigned int mmapGpio::GPPUDCLK1;
...
const unsigned int mmapGpio::PUD_OFF;
const unsigned int mmapGpio::PUD_DOWN;
const unsigned int mmapGpio::PUD_UP;
...
/*******************************************************************
 * setPinPUD() - sets the pullup resistor to up or down
 * output
 * 
 * Parameters - pinnum - GPIO pin number as per the RPI's  BCM2835's
 *                       standard definition
 ...
 * Return Value -None
 *
 * Page 101 - Broadcom 2835 Docs for relevant info
 * *****************************************************************/
void mmapGpio::setPinPUD(unsigned int pinnum, const unsigned int &pud)
{
    *(this->gpio + GPPUD) = 0x3 & pud;
    usleep(5);

    unsigned int offset = (pinnum / 32) ? GPPUDCLK1 : GPPUDCLK0;
    *(this->gpio + offset) = 1 << (0x1f & pinnum);
    usleep(5);
